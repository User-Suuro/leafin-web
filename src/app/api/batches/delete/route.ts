...
export async function POST(req: Request) {
  try {
    const { type, batchId } = await req.json() as { type: "fish" | "plant"; batchId: number };
    if (!batchId || !type) return NextResponse.json({ error: "Invalid request" }, { status: 400 });
    ...
    return NextResponse.json({ error: "Failed to delete batch", details: (error as Error).message }, { status: 500 });
  }
}
